...
import { CANCELLATION_DEADLINE_HOURS } from "../config/constants";
import { canCancelBooking } from "../lib/date";
...
export class BookingService {
  ...
  async getActiveByFamilyId(familyId: string): Promise<BookingDto[]> {
  ...
  async getBookingById(bookingId: string): Promise<(BookingDto & { familyId: string }) | null> {
  ...
  async getBookingOptions(input: { familyId: string; childId: string; lessonId: string }) {
  ...
  async cancelBooking(input: { bookingId: string; familyId: string }) {
    const client = await pool.connect();
    ...
    try {
      ...
      const refunded = canCancelBooking(row.startsAt);
      ...
      return {
        ...
        deadlineHours: CANCELLATION_DEADLINE_HOURS,
      };
    } catch (error) {
      await client.query("ROLLBACK");
      throw error;
